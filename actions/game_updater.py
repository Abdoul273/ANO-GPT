...
def _launch_steam_url(exe: Path, url: str) -> None:
    if is_mac():
        ...
        kit.spawn(["xdg-open", url])
    else:
        kit.spawn([str(exe), url])
    ...
    if game_name:
        ...
        try:
            ...
            if is_mac():
                ...
                kit.spawn([str(epic_exe), url] if epic_exe else ["xdg-open", url])
            else:
                kit.spawn([str(epic_exe), url])
        ...
        try:
            if is_mac():
                kit.spawn(["open", "-a", "Epic Games Launcher"])
            elif is_linux():
                if epic_exe:
                    kit.spawn([str(epic_exe)])
                else:
