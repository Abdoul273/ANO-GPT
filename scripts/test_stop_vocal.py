...
"""
# Ce diagnostic matériel est manuel : sa collecte/import ne doit ni ouvrir
# le micro, ni changer le routage audio, ni lancer la synthèse vocale.
if __name__ == "__main__":
    ...
    speaker = kit.spawn(["bash", "-c", "for i in 1 2 3 4 5 6; do espeak-ng -v fr -s 150 'Je suis ANO GPT et je parle sans arrêt pour tester la coupure vocale, dis stop maintenant.'; done"])
    os.environ.pop("PULSE_SINK", None)
    time.sleep(0.8)
    # Tout ce qui joue passe par le puits AEC (espeak est un processus enfant).
    for line in subprocess.run(["pactl", "list", "sink-inputs", "short"], capture_output=True, text=True, timeout=5).stdout.splitlines():
        subprocess.run(["pactl", "move-sink-input", line.split()[0], "anogpt_speaker_aec"], timeout=5)
    print("lecture routée via AEC")
    print(kit.run(["pactl", "list", "source-outputs"], timeout=5).stdout)
    ...
    try:
        ...
        if speaker is not None:
            try:
                os.killpg(speaker, signal.SIGTERM)
            except ProcessLookupError:
                print("Le processus de test vocal est déjà terminé.")
        stream.stop(); stream.close(); app_listener.stop(); ar.disable_echo_cancel()
